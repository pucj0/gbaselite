...
if not exist "%CONFIG_FILE%" (
  ...
  echo Set a strong auth.password, then run start.bat again.
  set "EXIT_CODE=2"
  goto :finish
)

pushd "%APP_DIR%" >nul
"%GBASELITE_EXE%" start --config "%CONFIG_FILE%"
set "EXIT_CODE=%ERRORLEVEL%"
popd >nul
if not "%EXIT_CODE%"=="0" echo ERROR: GBaseLite failed to start. Review the message above and the logs directory.
goto :finish

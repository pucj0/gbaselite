...
const (
	...
	CurrentSnapshotFormatVersion uint16 = 4
)
...
var snapshotMigrations = map[uint16]snapshotMigration{
	...
	3: {
		...
		apply: migrateDecimalAndCollation,
	},
}
...
func migrateDecimalAndCollation(snapshot *StoreSnapshot) error {
	for databaseIndex := range snapshot.Databases {
		for tableIndex := range snapshot.Databases[databaseIndex].Tables {
			table := &snapshot.Databases[databaseIndex].Tables[tableIndex]
			normalized, err := NormalizeDecimalSnapshot(*table)
			if err != nil {
				return err
			}
			*table = normalized

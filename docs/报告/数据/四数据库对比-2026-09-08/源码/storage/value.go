...
// meaningful. Null values retain their declared type. Group aligned fields
// together to keep each value at 80 bytes on 64-bit platforms (formerly 88).
// Gob matches exported field names, so existing snapshots remain readable.
type Value struct {
	Type  DataType
	Text  string
	Date  time.Time
...
func ParseDataType(raw string) (DataType, error) {
	...
	switch t {
	case TypeInt, TypeBigInt, TypeFloat, TypeDouble, TypeDecimal, TypeVarchar, TypeText, TypeBoolean, TypeDate, TypeDateTime:
		return t, nil
...
func NewValue(dataType DataType, raw any) (Value, error) {
	...
	case TypeDecimal:
		...
		return Value{Type: dataType, Text: string(d)}, nil

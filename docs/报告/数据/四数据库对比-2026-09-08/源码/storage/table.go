...
type Index struct {
	Collations []string // Derived from persisted column collations; immutable.
	Name       string
	Columns    []string
	Unique     bool
	Primary    bool
...
func newTable(name string, columns []Column, validateColumnNames bool) (*Table, error) {
	...
	for i := range columnCopy {
		...
		if validateColumnNames || column.Type != TypeDecimal || column.SQLType != "" {
			column.SQLType = normalizeColumnSQLType(*column)
		}
		if err := validateColumnCollation(*column); err != nil {
			return nil, err
		}
		if column.Type == TypeDecimal && column.SQLType != "" {
			if _, _, _, err := DecimalColumnSpec(*column); err != nil {
				return nil, err
			}
		}
...
func (t *Table) addIndex(name string, columns []string, unique, primary bool) error {
	...
	t.indexRows[key] = t.sortedIndexPositionsLocked(definition)
	if primary {
...
func (t *Table) AlterColumn(oldName string, replacement Column) error {
	if err := validateColumnCollation(replacement); err != nil {
		return err
	}
	...
	for key, definition := range t.indexes {
		...
		definition.Collations = collationsForIndex(definition, nextColumns, nextColumnIndex)
...
func (t *Table) Snapshot() TableSnapshot {
	t.mu.RLock()
	defer t.mu.RUnlock()
	return TableSnapshot{coldRows: t.cold, Name: t.name, Comment: t.comment, CreatedAt: t.createdAt, UpdatedAt: t.updatedAt, Columns: append([]Column(nil), t.columns...), Indexes: t.indexesLocked(), Rows: cloneRows(t.rows), ForeignKeys: cloneForeignKeys(t.foreignKeys), CheckConstraints: append([]CheckConstraint(nil), t.checks...), AutoIncrementNext: t.autoIncrementNextLocked()}
}
...
func (t *Table) Insert(row Row) error {
	...
	// Unique keys were validated and added above. Maintain only the new
	// ordered position instead of rebuilding every existing index entry.
	for key, definition := range t.indexes {
		positions := t.indexRows[key]
		at := len(positions)
		if at > 0 && compareRowsByIndex(t.rows[positions[at-1]], rowCopy, definition, t.columnIndex) > 0 {
			at = sort.Search(len(positions), func(i int) bool {
				return compareRowsByIndex(t.rows[positions[i]], rowCopy, definition, t.columnIndex) > 0
			})
		}
		positions = append(positions, position)
		copy(positions[at+1:], positions[at:len(positions)-1])
		positions[at] = position
		t.indexRows[key] = positions
	}
...
func (t *Table) Visit(predicate Predicate, visitor func(Row) error) error {
	...
	defer t.mu.RUnlock()
	if t.cold != nil {
		return t.cold.visit(predicate, 0, -1, visitor)
	}
	for _, row := range t.rows {
		if predicate != nil && !predicate(row) {
			continue
...
func (t *Table) StreamProject(predicate Predicate, selected []int, offset, limit int, yield func([]any) error) error {
	return t.Stream(predicate, offset, limit, func(row Row) error {
		projected := make([]any, len(selected))
		for i, index := range selected {
			projected[i] = row[index].Interface()
		}
...
func (t *Table) Stream(predicate Predicate, offset, limit int, yield func(Row) error) error {
	...
	t.mu.RUnlock()
	if cold != nil {
		return cold.visit(predicate, offset, limit, yield)
	}
...
func indexKey(definition Index, columnIndex map[string]int, row Row) (string, bool) {
	...
	for ordinal, column := range definition.Columns {
		position := columnIndex[normalizeName(column)]
		value := row[position]
		if value.Null {
			return "", false
		}
		key = appendIndexValueKey(key, foldIndexValue(value, indexCollation(definition, ordinal)))
	}
...
func appendIndexValueKey(key []byte, value Value) []byte {
	...
	switch value.Type {
	case TypeDecimal:
		key = append(key, DecimalKey(Decimal(value.Text))...)
...
func (t *Table) LookupUnique(column string, value Value) (Row, bool, bool) {
	...
	} // Use LookupUniqueChecked for cold I/O errors.
	for key, definition := range t.indexes {
...
func (t *Table) rebuildIndexesLocked() error {
	...
	for key, definition := range t.indexes {
		positions := t.sortedIndexPositionsLocked(definition)
		next[key] = positions
	}
	t.indexRows = next
	return nil
...
func compareRowsByIndex(left, right Row, definition Index, columnIndex map[string]int) int {
	for ordinal, column := range definition.Columns {
		position := columnIndex[normalizeName(column)]
		if comparison := compareIndexValue(left[position], right[position], indexCollation(definition, ordinal)); comparison != 0 {
			return comparison
		}
	}
	return 0
...
func (t *Table) ScanIndex(scan IndexScan, predicate Predicate, offset, limit int) ([]Row, error) {
	...
	if t.cold != nil {
		var rows []Row
		err := t.cold.streamIndex(scan, predicate, offset, limit, func(row Row) error { rows = append(rows, row); return nil })
		return rows, err
	}
...
func validateValue(column Column, value Value) error {
	if !value.Null && column.Type == TypeDecimal {
		if column.SQLType == "" {
			_, err := ParseDecimal(value.Text)
			return err
		}
		normalized, err := DecimalColumnValue(column, value.Interface())
		if err != nil {
			return err
		}
		if normalized.Text != value.Text {
			return fmt.Errorf("%w: decimal value is not quantized to the column scale", ErrTypeMismatch)
		}
	}
...
func rowDataLength(row Row) int64 {
	...
	for _, value := range row {
		...
		case TypeDate:
			total += 3
		case TypeVarchar, TypeText, TypeDecimal:
...
// Column positions are resolved once, not once per comparison. Row position
// breaks ties explicitly, so an allocation-free unstable sort is sufficient.
func (t *Table) sortedIndexPositionsLocked(definition Index) []int {
	positions := make([]int, len(t.rows))
	for i := range positions {
		positions[i] = i
	}
	columns := make([]int, len(definition.Columns))
	for i, name := range definition.Columns {
		columns[i] = t.columnIndex[normalizeName(name)]
	}
	slices.SortFunc(positions, func(left, right int) int {
		for ordinal, column := range columns {
			if comparison := compareIndexValue(t.rows[left][column], t.rows[right][column], indexCollation(definition, ordinal)); comparison != 0 {
				return comparison
			}
		}
		if left < right {
...
func (t *Table) indexIntervalLocked(scan IndexScan) ([]int, int, int, error) {
	definition, exists := t.indexes[normalizeName(scan.Name)]
	if !exists {
		return nil, 0, 0, fmt.Errorf("%w: %q", ErrIndexNotFound, scan.Name)
	}
	if len(scan.EqualPrefix) > len(definition.Columns) || len(scan.EqualPrefix) == len(definition.Columns) && (scan.Lower != nil || scan.Upper != nil) {
		return nil, 0, 0, fmt.Errorf("invalid bounds for index %q", scan.Name)
	}
	...
	comparePrefix := func(row Row) int {
		for index, value := range scan.EqualPrefix {
			...
			if comparison := compareIndexValue(row[position], value, indexCollation(definition, index)); comparison != 0 {
				return comparison
			}
		}
		return 0
	...
	if len(scan.EqualPrefix) < len(definition.Columns) {
		...
		if scan.Lower != nil {
			...
			start += sort.Search(end-start, func(index int) bool {
				comparison := compareIndexValue(t.rows[positions[start+index]][columnPosition], bound.Value, indexCollation(definition, len(scan.EqualPrefix)))
	...
	return positions, start, end, nil
}

// CountIndex counts matching entries without materializing or copying rows.
// Without a residual predicate only the two interval boundaries are needed.
func (t *Table) CountIndex(scan IndexScan, predicate Predicate) (int, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if t.cold != nil {
		if predicate == nil {
			index, err := t.cold.openIndex(scan.Name)
			if err != nil {
				return 0, err
			}
			return index.Count(scan)
		}
		count := 0
		err := t.cold.streamIndex(scan, predicate, 0, -1, func(Row) error { count++; return nil })
		return count, err
	}
	positions, start, end, err := t.indexIntervalLocked(scan)
	if err != nil {
	...
	count := 0
	for _, position := range positions[start:end] {
		if predicate(t.rows[position]) {
			count++
		}
	}
	return count, nil
...
// AutoIncrementState returns an independent copy of the reservation counters.
func (t *Table) AutoIncrementState() map[string]int64 {
	t.mu.RLock()
	defer t.mu.RUnlock()
	return t.autoIncrementNextLocked()
}

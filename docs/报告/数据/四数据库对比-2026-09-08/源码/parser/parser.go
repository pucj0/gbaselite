...
func (p *Parser) parseStatement() (Statement, error) {
	...
	case p.current().Kind == TokenEOF || p.current().Kind == TokenSemicolon:
		return Empty{}, nil
	case p.is("BACKUP") || p.is("RESTORE") || p.is("COMPACT") || p.is("GC"):
		kind := strings.ToUpper(p.current().Text)
		p.position++
		if err := p.expect("MVCC"); err != nil {
			return nil, err
		}
		if kind == "GC" {
			return MVCCMaintenance{Kind: kind}, nil
		}
		direction := "TO"
		if kind == "RESTORE" {
			direction = "FROM"
		...
		return MVCCMaintenance{Kind: kind, Path: path}, err
	...
	case p.accept("COMMIT"):
		return Commit{}, nil
	case p.accept("SAVEPOINT"):
		name, err := p.savepointName()
		return Savepoint{Name: name}, err
	case p.accept("RELEASE"):
		if err := p.expect("SAVEPOINT"); err != nil {
			return nil, err
		}
		name, err := p.savepointName()
		return ReleaseSavepoint{Name: name}, err
	...
	if p.accept("SELECT") {
		...
		for {
			...
			for {
				start := p.position
				value, err := p.literal()
				if err != nil || (p.current().Kind != TokenComma && p.current().Kind != TokenRParen) {
					p.position = start
					expression, expressionErr := p.parseExpr(0)
					if expressionErr != nil {
						return nil, expressionErr
					}
					if statement.ValueExpressions == nil {
						statement.ValueExpressions = make(map[[2]int]Expr)
					}
					statement.ValueExpressions[[2]int{len(statement.Values), len(row)}] = expression
					value = Literal{Kind: LiteralNull}
				}
...
func (p *Parser) parseColumnDefinition() (ColumnDef, error) {
	...
	for p.current().Kind != TokenEOF && p.current().Kind != TokenComma && p.current().Kind != TokenRParen {
		...
		case p.accept("COLLATE"):
			...
			column.Collation = strings.ToLower(name)
		default:
...
func (p *Parser) savepointName() (string, error) {
	token := p.current()
	if token.Kind != TokenIdentifier || token.Text == "" {
		return "", p.errorf("expected savepoint name")
	}
	if len([]rune(token.Text)) > 64 {
		return "", p.errorf("savepoint name exceeds 64 characters")
	}
	p.position++
	return token.Text, nil
}

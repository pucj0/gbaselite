...
func Lex(sql string) ([]Token, error) {
	runes := []rune(sql)
	// A large string literal is one token, not one token per two runes.
	// Bound speculative allocation; token-heavy SQL grows on demand.
	tokens := make([]Token, 0, min(len(runes)/2, 256))
	for i := 0; i < len(runes); {
		r := runes[i]

...
func NewSeed() ([]byte, error) {
	// Connector/J decodes the native-password challenge as ASCII. Use uniform
	// printable ASCII while retaining crypto/rand; raw high-bit bytes are lossy
	// in that driver and cause valid passwords to fail authentication.
	seed := make([]byte, 20)
	position := 0
	for position < len(seed) {
		var entropy [32]byte

...
func TestOKPacketLegacyOmitsOptionalInfo(t *testing.T) {
	got := OKPacketWithCapabilities(2, "table created", ClientProtocol41)
	want := []byte{0, 2, 0, 2, 0, 0, 0}
...
func TestTransactionStatusInOKAndEOF(t *testing.T) {
	r := &executor.Result{InTransaction: true}
	if resultStatus(r)&1 == 0 {
		t.Fatal("missing transaction status")
	}
	eof := resultEOFPacket(r)
	if binary.LittleEndian.Uint16(eof[3:])&1 == 0 {
		t.Fatal("EOF transaction status missing")
	}
	r.InTransaction = false
	if resultStatus(r)&1 != 0 {
		t.Fatal("transaction flag retained after commit")
	}
}

func TestAutocommitStatusIndependentOfTransaction(t *testing.T) {
	for _, c := range []struct {
		off, active bool
		want        uint16
	}{{false, false, 2}, {false, true, 3}, {true, false, 0}, {true, true, 1}} {
		if got := resultStatus(&executor.Result{AutocommitDisabled: c.off, InTransaction: c.active}); got != c.want {
			t.Fatal(c, got)
		}
	}
}

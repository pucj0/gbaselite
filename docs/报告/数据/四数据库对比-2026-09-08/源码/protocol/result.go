...
func okPacketWithStatus(affected, lastInsertID uint64, message string, capabilities uint32, status uint16) []byte {
	...
	// The legacy info field is optional. Omit it because Connector/J 9 also
	// attempts length-encoded decoding without negotiating session tracking.
	return data
}
...
func WriteResult(connection *PacketConn, result *executor.Result, schema, table string) error {
	...
	if err := connection.WritePacket(resultEOFPacket(result)); err != nil {
		return err
	}
	...
	return connection.WritePacket(resultEOFPacket(result))
}
...
func appendLenEncStorageValue(dst []byte, value storage.Value) []byte {
	...
	case storage.TypeDecimal, storage.TypeVarchar, storage.TypeText:
...
func appendColumnDefinition(data []byte, column executor.Column, schema, table string) []byte {
	...
	case storage.TypeDecimal:
		typeCode = TypeNewDecimal
		length = 67
		if column.SQLType != "" {
			if precision, scale, _, err := storage.DecimalColumnSpec(storage.Column{Type: storage.TypeDecimal, SQLType: column.SQLType}); err == nil {
				length = uint32(precision + 1)
				if scale > 0 {
					length++
				}
			}
	...
	if column.Type == storage.TypeDecimal {
		decimals = 31
		if column.SQLType != "" {
			if _, scale, _, err := storage.DecimalColumnSpec(storage.Column{Type: storage.TypeDecimal, SQLType: column.SQLType}); err == nil {
				decimals = byte(scale)
			}
		}
	}
	data = append(data, typeCode, byte(flags), byte(flags>>8), decimals, 0, 0)
	return data
}
...
func resultEOFPacket(result *executor.Result) []byte {
	status := resultStatus(result)
	return []byte{0xfe, 0, 0, byte(status), byte(status >> 8)}
}

// ResultEOFPacket preserves transaction mode in command metadata terminators.
func ResultEOFPacket(result *executor.Result) []byte { return resultEOFPacket(result) }

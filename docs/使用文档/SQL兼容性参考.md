...
读 [SQL 使用教程](SQL使用教程.md)；安装、配置、部署和备份见项目
[README](../../README.md)。
...
支持 `BEGIN`、`COMMIT` 和 `ROLLBACK`。默认显式事务使用实例级独占事务门：当前会话
读取自己的事务快照，其他会话的读取和写入等待事务结束。当前没有可选择的隔离级别、
行级锁、死锁检测或锁等待超时。

`SELECT ... FOR UPDATE` 语法可用于业务事务，但当前正确性来自实例级事务门，并不是
只锁命中行。支持真实 `SAVEPOINT name`、`ROLLBACK [WORK] TO [SAVEPOINT] name` 和
`RELEASE SAVEPOINT name`。名称不区分大小写、最多 64 字符；每事务最多 32 个，同名
替换旧位置。回滚保留目标、删除其后保存点；释放只删除目标。事务外 SAVEPOINT 不保留，
回滚或释放不存在的点返回 1305。提交、完整回滚和断连释放全部保存点。

保存点可恢复普通 DML、外键级联以及建表/视图/索引、增加列等受支持 DDL；当前 DDL
不会采用 MySQL 的隐式提交。存在保存点时拒绝 DROP DATABASE/TABLE、TRUNCATE、
表重命名、列 DROP/MODIFY/CHANGE/RENAME（含批量 ALTER），须先释放全部保存点。
账号/授权、USE、会话变量和 LAST_INSERT_ID 不回滚；不支持用户 CREATE TEMPORARY TABLE。
保存点不触发落盘，复制的行头/元数据仍占内存；回滚需恢复表并重建索引。
...
每连接默认最多 128 条预编译语句，保留 SQL 和参数类型向量的预算为 1024 KiB；
由 `server.max_prepared_statements` / `server.max_prepared_memory_kb` 配置。
达到预算返回 MySQL 1461，关闭不再使用的语句后可继续准备；占位符超过 65535 返回
1390。预算不包含执行结果、AST 和连接对象。见 README 的低资源运行说明。
...
简单 AND 比较条件下，单表 UPDATE/DELETE 可使用现有等值/范围索引候选；仍复核
WHERE 并保留物理选行顺序，文本条件按原排序规则限制回退。仅构建表达式字段元数据。
外键键值不变的单表变更阶段支持增量索引维护；级联及多表操作仍使用通用原子路径。
语句级快照和持久化流程未移除，这不是磁盘 B-tree 或行级锁实现。

未匹配排序索引的普通/表达式 SELECT，在 OFFSET+LIMIT <= 65536 且小于源行数一半时
使用稳定 Top-K；大分页回退全排序。NULL、排序规则和同值顺序保持一致。DISTINCT、
分组、窗口的结果不能由这个优化提前截断；关联/派生表等源关系的物化内存仍可能存在。

## JSON 函数兼容

支持 JSON_OBJECT/ARRAY、QUOTE/UNQUOTE、EXTRACT、SET/INSERT/REPLACE/REMOVE、VALID/TYPE/LENGTH/DEPTH/KEYS 和 CONTAINS_PATH 共 15 个函数。JSON 列仍为文本，普通 SQL 字符串不会自动变成嵌套对象。VALUES 可使用标量表达式；含表达式的多行 VALUES 及 INSERT SELECT 在语句快照上执行，错误时不发布部分插入，但需要额外内存和独占语句锁。纯字面量多行 INSERT 同样整句失败回滚。详见 [JSON 兼容范围、路径限制和示例](JSON兼容性说明.md)。不支持完整 MySQL JSON 运算、索引和二进制存储语义。

本轮新增精确DECIMAL、显式列COLLATE、可选分页冷读、查询资源预算及乐观事务，
详见 [资源控制与限制](查询资源预算.md)。默认事务仍使用原独占方式；
乐观模式需要显式配置，并按整个实例的提交版本检测写冲突。快照格式已经升至4。
分页实例的完整备份必须包含整个数据目录，inspect-instance目前拒绝分页目录，
不能将残留旧gob解释为当前数据；应在隔离副本以paged模式实际恢复验证。

...
GBaseLite 支持 `BEGIN`、`COMMIT` 和 `ROLLBACK`。默认显式事务会取得实例级独占事务门，
同一会话读取自己的事务快照；其他会话的读取和写入都等待到 `COMMIT`、`ROLLBACK` 或
连接断开自动回滚后继续。当前没有可选择的隔离级别，这一行为比 MySQL 默认的
`REPEATABLE READ` 更保守，因此事务必须保持短小。

需要只撤销事务的一部分时，可以使用保存点：
...
保存点名称不区分大小写，最多 64 字符，每事务最多 32 个；同名替换。回滚到保存点会
删除更晚的点并保留目标，RELEASE 只删除目标；不存在返回 1305。已消耗的自增 ID 不复用。
事务外 SAVEPOINT 不保留。保存点共享不可变行载荷，但仍复制行头及元数据，保存点越多，
占用越大；回滚需要重建索引。请尽早释放不再需要的点。

存在保存点时，DROP DATABASE/TABLE、TRUNCATE、表重命名和列的
DROP/MODIFY/CHANGE/RENAME 会报错，应先释放全部保存点；建表、建索引、增加列等
受支持 DDL 可回滚，且不采用 MySQL DDL 隐式提交。账号/权限、USE、会话变量和
LAST_INSERT_ID 不受保存点回滚影响，不支持用户 CREATE TEMPORARY TABLE。
...
`databases/store.gob` 当前是格式版本 `4`，`users/users.gob` 是格式版本 `1`。没有

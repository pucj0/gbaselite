...
当前版本：`1.1.5`（工作区开发改造；历史发布包以对应发布说明为准）
...
- 表/视图 DDL 与受支持 CRUD（含 INSERT SET/IGNORE、REPLACE、ON DUPLICATE KEY、UPDATE JOIN、INSERT SELECT、多表 DELETE）、聚合、分组、INNER/LEFT/RIGHT/CROSS JOIN、派生表、非递归与递归 CTE、视图、标量/IN/EXISTS 子查询
...
| 事务 | BEGIN/COMMIT/ROLLBACK、SAVEPOINT/ROLLBACK TO/RELEASE、SET autocommit=0/1、断连回滚、语句失败回滚 | 快照隔离；不支持 LOCK TABLES、隔离级别切换或串行化保证；每个事务最多 32 个存活 savepoint layer（`RELEASE`/同名替换只释放名字，匿名 layer 仍占名额，超限报 resource limit） |
...
UNIQUE、CHECK、外键或类型转换失败都会回滚整条语句。多表 DELETE 按下表的物理行 identity 去重
（无主键表用扫描得到的 storage row key，主键表用编码后的主键），并按“先删引用方（子表）、后删
被引用方（父表）”的顺序执行，使同一条语句同时删除父表与子表时仍满足 RESTRICT 外键；它不支持
LIMIT，目标表可以没有主键（含 joined 目标），目标表之间不允许循环外键，连接输入与查询源必须是
已支持的基表/视图查询；派生表与 CTE 没有可删除的物理 identity，不能作为 DELETE 目标（同 legacy）。
...
支持 JOIN、聚合、UNION、派生表、子查询、嵌套视图、别名与限定列，也可作为 `INSERT SELECT` 的源；
...
ON UPDATE 列表达式、MVCC 不支持的外键及无效数据会明确报错，不丢弃结构或
...
据库、表/索引、视图、事务回滚、默认 MVCC、`SHOW DATABASES`、自包含导出、删除后导入和恢复结果核对。该任
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\build-release.ps1 `
  -Version 1.1.5 -GoExecutable D:\env\Go\bin\go.exe
```

兼容入口 `scripts/package.ps1` 会转发到同一脚本。Linux 构建机可以使用：

```bash
VERSION=1.1.5 ./scripts/build-release.sh
```
...
.\scripts\build-msi.ps1 -Version 1.1.5 `
...
.\publish-release.bat -Version 1.1.5 -DryRun -ReplaceArtifacts
.\publish-release.bat -Version 1.1.5 -PrepareOnly -ReplaceArtifacts
.\publish-release.bat -Version 1.1.5 -Publish -ReplaceArtifacts
```
...
Engine 最小接口为 Begin(ctx)/Close；RevisionReader、CounterAllocator、ReplicatedEngine、Availability、Maintenance、Diagnostics、TransactionDiagnostics 均独立。缺失 RevisionReader 时通过普通 Txn 快照刷新元数据；自增/维护等显式能力缺失返回 ErrUnsupported。FullEngine 保留原完整方法集合用于嵌入方的兼容迁移。已有完整后端无需修改实现。

事务诊断为可选能力 TransactionDiagnostics，ActiveTransactions/TransactionStats 返回只读快照：状态取 ACTIVE/COMMITTING/COMMITTED/ABORTED/MERGED，无法判定的状态一律映射为 UNKNOWN 而不是 ACTIVE；只读字段为计数与类别，不含键、值与 SQL 文本。该能力不改变事务语义、可见性、冲突判定与 GC，也不进入 SQL 层；仅 MVCC 后端（含本地 WAL 配置）提供，内存后端不提供，缺失时按不存在的能力处理。TransactionStats.Conflicts 统计真实冲突中止（此前恒为 0）。
...
执行层按职责命名：transaction_engine、access_plan、row_layout 等模块只依赖通用存储接口；原 SetMVCCAutocommit Go API 保留为 SetAutocommit 的兼容转发。协议消息与持久化编码不变；不支持特性的错误文本已去除具体后端前缀，不改变错误类型或 SQL 执行结果。审计见 docs/architecture/executor-dependency-audit.md。
...
A04 SQL 能力审计矩阵见 [docs/设计/A04-MVCC-SQL能力矩阵.md](docs/设计/A04-MVCC-SQL能力矩阵.md)。

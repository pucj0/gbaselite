...
func (s *MySQLServer) handleConnection(raw net.Conn) {
	...
	if len(responseData) == 32 && binary.LittleEndian.Uint32(responseData[:4])&protocol.ClientSSL != 0 {
		...
		packet = &protocol.PacketConn{Conn: tlsConnection, Sequence: packet.Sequence, MaxReadBytes: packet.MaxReadBytes}
		packet.EnableWriteBuffer(writeBufferSize)
		responseData, err = packet.ReadPacket()
		if err != nil {
			return
	...
	for {
		...
		case 0x16:
			...
			statementID, statement, code, prepareErr := prepared.add(query)
			if prepareErr != nil {
				_ = packet.WritePacket(protocol.ErrorPacket(code, prepareErr.Error()))
				break
			}
			...
			if parameterCount > 0 {
				...
				_ = packet.WritePacket(protocol.ResultEOFPacket(&executor.Result{InTransaction: session.InTransaction(), AutocommitDisabled: session.AutocommitDisabled}))
			}
		case 0x17:
			started := time.Now()
			...
			statement := prepared.statements[statementID]
			if statement == nil {
		...
		case 0x1f: // COM_RESET_CONNECTION
			s.Engine.ResetConnection(session)
			prepared = newPreparedCache(prepared.maxCount, prepared.maxBytes)
			_ = protocol.WriteResult(packet, &executor.Result{}, session.CurrentDatabase, "")
		case 0x19:
			if len(commandData) >= 5 {
				prepared.remove(binary.LittleEndian.Uint32(commandData[1:5]))
			}
		case 0x1a:
			_ = protocol.WriteResult(packet, &executor.Result{InTransaction: session.InTransaction(), AutocommitDisabled: session.AutocommitDisabled}, session.CurrentDatabase, "")
		default:
...
func queryForLog(query string) string {
	...
	for position := range query {
		if count == maxRunes {
			return query[:position] + fmt.Sprintf("... [truncated, %d runes total]", utf8.RuneCountInString(query))
		}
		count++
	}
	return query

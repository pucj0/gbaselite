...
type Insert struct {
	...
	Columns          []string
	Values           [][]Literal
	ValueExpressions map[[2]int]Expr // Sparse overrides keep literal-only inserts allocation-light.
	SetValues        []Expr
	Select           Query
	Ignore           bool
	Replace          bool
...
type RollbackTo struct{ Name string }
...
func (RollbackTo) statement()       {}
func (ReleaseSavepoint) statement() {}

// MVCCMaintenance is a GBaseLite extension, not MySQL syntax.
type MVCCMaintenance struct{ Kind, Path string }

func (MVCCMaintenance) statement() {}
